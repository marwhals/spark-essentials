package part6_how_spark_runs_on_a_cluster

import org.apache.spark.sql.SparkSession

//Reference code/commands from "How Spark Runs on a cluster" section.

object SparkJobAnatomy extends App {
  /**
   * This is the code we wrote during the Spark Job Anatomy section.
   * This code is not meant to be run standalone (although doable).
   * It was mostly used to deconstruct Spark jobs and exemplify stages, tasks and the DAG.
   *
   * Keep code for reference
   * You can copy into the Spark shell.
   */
  ...
  rdd1.toDF.show
  // suddenly from one step we get 5 steps: converting to DataFrame does a lot of things behind the scenes
  ...
  /**
   * Complex job 2
   * This executes a single job with a very complicated DAG, and 6 stages:
